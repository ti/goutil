...
func TestTTL(t *testing.T) {
	m := New()
	m.SetTTL("a",map[string]bool{"v":true}, time.Second)
	if a, ok := m.Get("a").(map[string]bool); !ok && !a["v"] {
		t.Fatal("cache does not match")
	}
	time.Sleep(time.Second)
	if _, ok := m.Get("a").(map[string]bool); ok {
		t.Fatal("cache does not expired")
	}
	m.Remove("a")
	if _, ok := m.Get("a").(map[string]bool); ok {
		t.Fatal("cache does not deleted")
	}
}
...
func TestPop(t *testing.T) {
	...
	if !ok || m1 != monkey {
		t.Error("Pop found something else, but monkey.", v)
	}
...
func TestUpsert(t *testing.T) {
	...
	if m.Count() != 2 {
		t.Error("map should contain exactly two elements, bus is",m.Count())
	}
	...
	marineAnimals := m.Get("marine")
	if !compare(marineAnimals.([]Animal), []Animal{dolphin, whale}) {
		t.Error("Set, then Upsert failed")
	}

	predators := m.Get("predator")
	if !compare(predators.([]Animal), []Animal{tiger, lion}) {
